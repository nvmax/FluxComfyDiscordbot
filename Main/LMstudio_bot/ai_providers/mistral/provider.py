            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(f"Mistral API connection test failed: {error_text}")
                        return False
...
Creativity level: {creativity_level}/10 (higher means more creative additions)"""
    ...
    async def generate_response(self, prompt: str, temperature: float = 0.7) -> str:
        """Generate an enhanced prompt using Mistral's API."""
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload, timeout=30) as response:
                    if response.status != 200:
                        error_text = await response.text()
                        raise Exception(f"Mistral API error: HTTP {response.status} - {error_text}")
